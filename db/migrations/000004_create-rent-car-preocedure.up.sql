create or replace procedure rent_car(inout requisitionId uuid, carId uuid, consumerId uuid, dateFrom timestamptz, dateTo timestamptz)
  language plpgsql
as $$
declare
  foundCarId uuid;
...
begin
  ...
  requisitionId = uuid_generate_v4();
  insert into requisitions (id, car_id , consumer_id , date_from , date_to , created_at )
  values(requisitionId, carId, consumerId, dateFrom , dateTo, now());
  commit;
