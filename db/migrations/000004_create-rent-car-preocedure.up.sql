...
begin
	...
	if not found then raise exception 'Car id % was not found', carId;
    end if;

   	select c.id into foundConsumerId from consumers c where c.id=consumerId;
	if not found then raise exception 'Consumer id % was not found', consumerId;
    end if;
    ...
    if found then raise exception 'Provided period is overlapsing';
    end if;
